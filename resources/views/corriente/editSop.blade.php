    ...
    <div class="col-md-12">
            <div class="card shadow-sm mb-3 bg-white rounded" style="max-width: 550px;">
                ...
                <div class="card-body">
                        ...
                        @endif
                    @isset($sopa)
                    <form action="{{ route('sopa.update',$sopa->id)}}" method="POST">
                        @method('PUT')
                        @csrf
                        <label for="precio">Nombre:</label>
                        <input type="text" class="form-control" name="name" placeholder="Nombre de la sopa" value="{{old('nombre',$sopa->name)}}">
                        ...
                        <div class="d-flex justify-content-around pt-2 mt-4">
                            <a href="{{ route('sopa.create')}}" class="btn btn-outline-warning block-center">Cancelar</a>
